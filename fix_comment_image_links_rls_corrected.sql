-- Fix RLS policies for comment_image_links table (CORRECTED VERSION)
-- Fixed column names: comments table uses 'created_by' not 'user_id'
...
WITH CHECK (
  comment_id IN (
    SELECT id FROM comments WHERE created_by = auth.uid()
  )
);
