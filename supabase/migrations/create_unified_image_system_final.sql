-- =====================================================
-- Unified Image Management System Migration (FINAL - SAFE)
...
-- Step 4: Create unified managed_images table (safe)
...
-- Step 5: Create POI image links table (safe)
CREATE TABLE IF NOT EXISTS poi_image_links (
...
-- Step 6: Create comment image links table (safe)
CREATE TABLE IF NOT EXISTS comment_image_links (
...
-- Step 7: Create item image links table (safe)
...
-- Step 8: Create schematic image links table (safe)
CREATE TABLE IF NOT EXISTS schematic_image_links (
    entity_id UUID REFERENCES entities(id) ON DELETE CASCADE,
    image_id UUID REFERENCES managed_images(id) ON DELETE CASCADE,
    display_order INTEGER DEFAULT 0,
    created_at TIMESTAMPTZ DEFAULT NOW(),
...
-- Step 9: Create indexes for performance (safe)
CREATE INDEX IF NOT EXISTS idx_managed_images_type ON managed_images(image_type);
CREATE INDEX IF NOT EXISTS idx_managed_images_uploaded_by ON managed_images(uploaded_by);
CREATE INDEX IF NOT EXISTS idx_managed_images_created_at ON managed_images(created_at);
CREATE INDEX IF NOT EXISTS idx_poi_image_links_poi ON poi_image_links(poi_id);
CREATE INDEX IF NOT EXISTS idx_poi_image_links_display_order ON poi_image_links(poi_id, display_order);
CREATE INDEX IF NOT EXISTS idx_comment_image_links_comment ON comment_image_links(comment_id);
CREATE INDEX IF NOT EXISTS idx_item_image_links_entity ON item_image_links(entity_id);
CREATE INDEX IF NOT EXISTS idx_item_image_links_display_order ON item_image_links(entity_id, display_order);
CREATE INDEX IF NOT EXISTS idx_schematic_image_links_entity ON schematic_image_links(entity_id);
CREATE INDEX IF NOT EXISTS idx_schematic_image_links_display_order ON schematic_image_links(entity_id, display_order);

-- Step 10: Create constraints (safe)
DO $$ 
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint 
        WHERE conname = 'valid_image_type' 
        AND conrelid = 'managed_images'::regclass
    ) THEN
...
-- Step 12: Drop existing policies safely before creating new ones
DO $$
BEGIN
...
-- Step 13: Create RLS policies for managed_images
...
-- Step 14: Create RLS policies for POI image links
...
-- Step 15: Create RLS policies for comment image links
...
-- Step 16: Create RLS policies for item image links
...
-- Step 17: Create RLS policies for schematic image links
...
-- Step 18: Create updated_at trigger for managed_images (safe)
...
DROP TRIGGER IF EXISTS update_managed_images_updated_at ON managed_images;
CREATE TRIGGER update_managed_images_updated_at 
    BEFORE UPDATE ON managed_images 
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

-- Step 19: Add comments (safe)
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_description 
        WHERE objoid = 'managed_images'::regclass
    ) THEN
