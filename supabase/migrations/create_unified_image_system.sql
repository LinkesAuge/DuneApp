-- =====================================================
-- Unified Image Management System Migration
...
-- Step 9: Add RLS policies for managed_images
    ...
    FOR SELECT USING (
        ...
        EXISTS (
            ...
            AND (p.privacy_level = 'public' OR p.created_by = auth.uid() OR
                EXISTS (SELECT 1 FROM poi_shares ps WHERE ps.poi_id = p.id AND ps.shared_with = auth.uid()))
        ) OR
        EXISTS (
...
-- Step 11: Add RLS policies for link tables
...
-- Step 11: Create updated_at trigger for managed_images
